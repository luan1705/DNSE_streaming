...
def is_trading_time_vn():
    ...
    # Trước 9h, nghỉ trưa 11h30-13h, sau 14h45
    if (
        hm < 9 or
        11.5 <= hm < 13 or
        hm > 14.75
...
def upsert_1m(mapped_symbol, data):
    try:
        ...
        print(f"✅ [1M][DB] {mapped_symbol} @ {time_vn_str} VN")

    except Exception as e:
        print(f"⚠️ DB error {mapped_symbol}: {e}")
...
BROKER_PORT = 443
client_id = f"dnse-ohlc-derivatives-1m-{randint(1000,9999)}"
...
def on_message(client, userdata, msg):
    try:
        ...
        # FIX: Giống code 1 - Timestamp từ DNSE đã là VN time
        time_vn = datetime.fromtimestamp(ts)
        time_vn = time_vn.replace(tzinfo=VN_TZ)
        time_vn = time_vn.replace(second=0, microsecond=0)
        time_vn_str = time_vn.strftime("%Y-%m-%d %H:%M:%S")

        print(f"📥 [{raw_symbol}] -> [{mapped_symbol}] {time_vn_str} VN | O={data.get('open')} C={data.get('close')}")
...
def redis_subscriber_test():
    ...
    print(f"👂 Redis subscriber: {REDIS_CHANNEL}")
    for message in sub.listen():
        if message["type"] == "message":
            print(f"Redis: {message['data'].decode()}")
